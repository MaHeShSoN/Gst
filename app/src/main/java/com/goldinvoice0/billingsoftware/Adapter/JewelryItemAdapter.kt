...
import com.goldinvoice0.billingsoftware.databinding.RvItemJewellery00Binding

class  JewelryItemAdapter(
    private val onEditClick: (JewelryItem, Int) -> Unit,
    private val onDeleteClick: (JewelryItem, Int) -> Unit,
    private val onStatusChange: (JewelryItem, Int) -> Unit
) : ListAdapter<JewelryItem, JewelryItemAdapter.JewelryViewHolder>(JewelryDiffCallback()) {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): JewelryViewHolder {
        val inflater = LayoutInflater.from(parent.context)
        val binding = RvItemJewellery00Binding.inflate(inflater, parent, false)
    ...
    inner class JewelryViewHolder(
        private val binding: RvItemJewellery00Binding
        ...
        fun bind(item: JewelryItem, position: Int) {
            binding.apply {
                ...
                statusChip.setOnClickListener {
                    showStatusDialog(item, position)
                }
            }
        }
        ...
        private fun showStatusDialog(item: JewelryItem, position: Int) {
            val statuses = DeliveryStatus.values().map { it.name }
            val currentIndex = statuses.indexOf(item.status.name)
            ...
            AlertDialog.Builder(itemView.context)
                ...
                .setSingleChoiceItems(
                    ...
                    val newStatus = DeliveryStatus.valueOf(statuses[which])
                    onStatusChange(item.copy(status = newStatus), position)
                    dialog.dismiss()
                }
                .show()
        }
